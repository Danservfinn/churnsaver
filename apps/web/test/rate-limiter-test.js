...
const { checkRateLimit, RATE_LIMIT_CONFIGS } = require('../src/server/middleware/rateLimit');
...
describe('Rate Limiter - Concurrent Scenarios', () => {
  ...
  describe('Fixed Time Bucketing', () => {
    it('should use fixed time buckets for all requests in same window', async () => {
      ...
      mockSql.select.mockResolvedValue([{ count: 2 }]);
      ...
      expect(mockSql.select).toHaveBeenCalledTimes(2);
      expect(mockSql.select.mock.calls[0][1]).toEqual([identifier, expectedBucketStart]);
      expect(mockSql.select.mock.calls[1][1]).toEqual([identifier, expectedBucketStart]);
    ...
    it('should use different buckets for different time windows', async () => {
      ...
      mockSql.select.mockResolvedValue([{ count: 0 }]);
      mockSql.execute.mockResolvedValue(undefined);

      await checkRateLimit(identifier, config);
      
      const firstBucketStart = new Date('2023-01-01T12:00:00.000Z');
      expect(mockSql.select.mock.calls[0][1]).toEqual([identifier, firstBucketStart]);
      ...
      expect(mockSql.select.mock.calls[1][1]).toEqual([identifier, secondBucketStart]);
  ...
  describe('Composite Primary Key Usage', () => {
    ...
    it('should handle concurrent requests to same bucket correctly', async () => {
      ...
      mockSql.select.mockResolvedValueOnce([{ count: 0 }]);
      mockSql.execute.mockResolvedValueOnce(undefined);
      ...
      mockSql.select.mockResolvedValueOnce([{ count: 1 }]);
      mockSql.execute.mockResolvedValueOnce(undefined);
      ...
      mockSql.select.mockResolvedValueOnce([{ count: 2 }]);
  ...
  describe('Cleanup Functionality', () => {
    ...
    it('should not delete active buckets', async () => {
      ...
      expect(mockSql.execute.mock.calls[0][0]).toContain('< $1');
  ...
  describe('Reset Time Calculation', () => {
    it('should calculate reset time as start of next bucket', async () => {
      ...
      mockSql.select.mockResolvedValue([{ count: 5 }]); // At limit
