...
-- Add company_id to migration_tracking table (from 011_migration_tracking.sql)
ALTER TABLE migration_tracking ADD COLUMN IF NOT EXISTS company_id text NOT NULL DEFAULT 'biz_hqNeRcxEMkuyOL';
CREATE INDEX IF NOT EXISTS idx_migration_tracking_company ON migration_tracking(company_id);
...
-- AB Tests policies
CREATE POLICY ab_tests_company_select_policy ON ab_tests
FOR SELECT USING (company_id = get_current_company_id());

CREATE POLICY ab_tests_company_insert_policy ON ab_tests
FOR INSERT WITH CHECK (company_id = get_current_company_id());

CREATE POLICY ab_tests_company_update_policy ON ab_tests
...
CREATE POLICY ab_test_participants_company_select_policy ON ab_test_participants
FOR SELECT USING (company_id = get_current_company_id());

CREATE POLICY ab_test_participants_company_insert_policy ON ab_test_participants
FOR INSERT WITH CHECK (company_id = get_current_company_id());

CREATE POLICY ab_test_participants_company_update_policy ON ab_test_participants
FOR UPDATE USING (company_id = get_current_company_id());

-- Job Queue policies
CREATE POLICY job_queue_company_select_policy ON job_queue
FOR SELECT USING (company_id = get_current_company_id());

CREATE POLICY job_queue_company_insert_policy ON job_queue
FOR INSERT WITH CHECK (company_id = get_current_company_id());

CREATE POLICY job_queue_company_update_policy ON job_queue
FOR UPDATE USING (company_id = get_current_company_id());
...
CREATE POLICY migration_tracking_company_select_policy ON migration_tracking
FOR SELECT USING (company_id = get_current_company_id());

CREATE POLICY migration_tracking_company_insert_policy ON migration_tracking
FOR INSERT WITH CHECK (company_id = get_current_company_id());
...
BEGIN
  ...
  UPDATE migration_tracking SET company_id = 'biz_hqNeRcxEMkuyOL' WHERE company_id IS NULL;
...
ALTER TABLE migration_tracking ALTER COLUMN company_id SET NOT NULL;
